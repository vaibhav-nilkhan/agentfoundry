 ...
 *
 * Usage: ts-node validate-skill.ts <skill-path>
 */
...
class SkillValidator {
  ...
  private async validateManifest(): Promise<void> {
    ...
    try {
      ...
      const manifest: any = yaml.load(content);
      ...
      if (manifest.description && manifest.description.length > 500) {
        this.warnings.push('description should be under 500 characters');
      }

      // Validate platforms
      const validPlatforms = ['web', 'desktop', 'mobile', 'api'];
      if (manifest.platforms && Array.isArray(manifest.platforms)) {
        for (const platform of manifest.platforms) {
          if (!validPlatforms.includes(platform)) {
            this.errors.push(`Invalid platform: ${platform}`);
          }
        }
      }
      ...
      if (manifest.tools && Array.isArray(manifest.tools)) {
        for (const tool of manifest.tools) {
          if (!tool.name || !tool.description || !tool.entry || !tool.input_schema) {
            this.errors.push(`Tool missing required fields: ${tool.name || 'unnamed'}`);
          }

          // Check tool entry file exists
          if (tool.entry) {
            const toolPath = path.join(this.skillPath, tool.entry);
            if (!fs.existsSync(toolPath)) {
              this.errors.push(`Tool entry file not found: ${tool.entry}`);
            }
          }
        }
      } else {
        this.errors.push('tools must be defined as an array');
      }

      // Validate pricing tiers
      if (manifest.pricing && manifest.pricing.tiers) {
        if (!Array.isArray(manifest.pricing.tiers) || manifest.pricing.tiers.length === 0) {
          this.errors.push('pricing.tiers must be a non-empty array');
        }
      }

      this.checks.manifest = this.errors.length === 0;
    } catch (error: any) {
      this.errors.push(`Failed to parse skill.yaml: ${error.message}`);
    }
  }

  private validateReadme(): void {
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: ts-node validate-skill.ts <skill-path>');
    process.exit(1);
  }
